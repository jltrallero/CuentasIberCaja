...
namespace GestorGastos.Frm
{
    public partial class GraficosxMesForm : Form
    {
        ...
        public GraficosxMesForm(IEnumerable<Expense> datos)
        {
            InitializeComponent();
            expenses = datos; 
        ...
        private void CrearGraficoComparativo(Control panel, IEnumerable<Expense> datos, bool esIngreso, TipoGrafico tipo)
        {
            var valores = datos
            ...
            foreach (var año in valores.Select(v => v.Año).Distinct().OrderBy(a => a))
            {
                ...
                {
                    var total = datosPorMes.TryGetValue(mes, out Dictionary<int, float>? value) && value.TryGetValue(año, out float value1)
                        ? value1 : 0;
